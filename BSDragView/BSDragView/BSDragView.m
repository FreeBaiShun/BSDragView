...
//  AFNetworking
//
//  Created by yuYue on 2018/11/8.
//

#import "BSDragView.h"
CGFloat const TopDistance = 40;
CGFloat const BottomDistance = 40;

#define BSSCREENWIDTH  [UIScreen mainScreen].bounds.size.width
#define BSSCREENHEIGHT [UIScreen mainScreen].bounds.size.height
// iphonX
#define BSIPhoneX ((BSSCREENWIDTH == 375.f || BSSCREENWIDTH == 414.f)  && (BSSCREENHEIGHT == 812.f || BSSCREENHEIGHT == 896.f) ? YES : NO)
#define BSIPhoneXBottomSafeFieldHeight 34

@interface BSDragView()<UIGestureRecognizerDelegate>
/**
    ...
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateChanged:{//拖动中
            ...
            switch (self.dragDirection) {
                case BSDragDirectionAny:
                    dx = point.x - self.startPoint.x;
                    dy = point.y - self.startPoint.y;
                    break;
                case BSDragDirectionHorizontal:
                    dx = point.x - self.startPoint.x;
                    dy = 0;
                    break;
                case BSDragDirectionVertical:
                    dx = 0;
                    dy = point.y - self.startPoint.y;
                    break;
                default:
...
- (void)keepBounds{
    ...
    CGFloat offsetBottom = 0.0;
    if (BSIPhoneX) {
        offsetTop = 44.0;
        offsetBottom = 34.0;
    }
    CGFloat distanceTop = TopDistance + offsetTop;
    CGFloat distanceBottom = BottomDistance +offsetBottom;
    ...
    if (self.isKeepBounds==NO) {//没有黏贴边界的效果
        ...
        if (self.isTopBottomKeepBounds) {
            //上下粘边
            CGFloat topDistance = self.frame.origin.y;
            CGFloat bottomDistance = self.freeRect.size.height - topDistance - self.bounds.size.height;
            if (topDistance <= distanceTop) {
                //上部粘边
                CGContextRef context = UIGraphicsGetCurrentContext();
                [UIView beginAnimations:@"topMove" context:context];
                [UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];
                [UIView setAnimationDuration:0.5];
                rect.origin.y = self.freeRect.origin.y+offsetTop;
                self.frame = rect;
                [UIView commitAnimations];
            }else if(bottomDistance <= distanceBottom){
                //下部粘边
                CGContextRef context = UIGraphicsGetCurrentContext();
                [UIView beginAnimations:@"bottomMove" context:context];
                [UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];
                [UIView setAnimationDuration:0.5];
                rect.origin.y = self.freeRect.origin.y+self.freeRect.size.height-self.frame.size.height-offsetBottom;
                self.frame = rect;
                [UIView commitAnimations];
            }else{
                //左右粘边
